...
import 'package:magic/utils/platform_provider.dart';
...
class HomeScreenViewModel extends ChangeNotifier {
  ...
  void refreshUser() {
    displayName = _magicSdk?.currentUser?.displayName ?? '';
    displayImage = _magicSdk?.currentUser?.photoURL;
    numberOfLinkedProviders =
        _magicSdk?.currentUser?.providerUserInfo?.length ?? 0;
    notifyListeners();
  }
  ...
  final magicAuth? _magicSdk = magicApp.magicAuth;
  ...
  Future<void> sendEmailVerificationCode(VoidCallback onSuccess) async {
    try {
      ...
      await _magicSdk?.sendEmailVerificationCode();
  ...
  Future<void> getAdditionalUserInfo() async {
    try {
      ...
      await _magicSdk?.getAdditionalUserInfo();
  ...
  Future<void> linkProvider() async {
    try {
      ...
      await _magicSdk?.linkProviderToUser(
        getPlatformId(),
        signInAuth!.idToken!,
      );
