...
void main() {
  group('MagicUtils.parseAuthorizationHeader', () {
    test('Extracts token from a valid Authorization header', () {
      ...
      expect(AortemMagicAuthHeaderParse.parseAuthorizationHeader(header), equals('abc123tokenXYZ'));
    });
    ...
    test('Throws ArgumentError for empty header', () {
      expect(() => AortemMagicAuthHeaderParse.parseAuthorizationHeader(''), throwsArgumentError);
    });

    test('Throws ArgumentError for missing Bearer prefix', () {
      const header = 'Token abc123tokenXYZ';
      expect(() => AortemMagicAuthHeaderParse.parseAuthorizationHeader(header), throwsArgumentError);
    });
